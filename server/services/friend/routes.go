...
func (h *Handler) GetFriendsForSidebar(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.WriteError(w, http.StatusNotFound, err, "Failed to get friends")
		return
	}
...
func (h *Handler) GetFriendsStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.WriteError(w, http.StatusNotFound, err, "Not Found")
		return
	}
...
func (h *Handler) UpdateFriendStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err := utils.Validate.Struct(payload); err != nil {
		...
		utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("validation error: %v", errors), "Failed to validate body")
		return
	}

	err = h.store.UpdateFriendStatus(userID, payload.FriendID, payload.Status)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, err, "Failed to update status")
	}
